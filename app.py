...
class QueryMancerApp:
    ...
    def _display_query_results(self, db_result: Dict[str, Any]):
        """Display query results in a beautiful, insightful, and eye-catching format"""
        if not db_result.get('success', False):
            st.error(f"Query failed: {db_result.get('error', 'Unknown error')}")
            return
        
        # Show execution info with beautiful metrics cards
        execution_time = db_result.get('execution_time', 0)
        rows_affected = db_result.get('rows_affected', 0)
        
        # Beautiful metrics header with animated gradient
        st.markdown("""
        ...
        <div class="metrics-grid">
            <div class="metric-card metric-time">
                ...
                <div class="metric-content">
                    <span class="metric-value">{execution_time:.3f}s</span>
                    <span class="metric-label">Execution Time</span>
                </div>
                <div class="metric-sparkline time-sparkline"></div>
            </div>
            <div class="metric-card metric-rows">
                <div class="metric-icon-wrapper">
                    <span class="metric-icon">📋</span>
                    <div class="metric-ring"></div>
                </div>
                <div class="metric-content">
                    <span class="metric-value">{rows_affected:,}</span>
                    <span class="metric-label">Rows Returned</span>
                </div>
                <div class="metric-sparkline rows-sparkline"></div>
            </div>
            <div class="metric-card metric-status">
                <div class="metric-icon-wrapper">
                    <span class="metric-icon">✅</span>
                    <div class="metric-ring success-ring"></div>
                </div>
                <div class="metric-content">
                    <span class="metric-value success-text">Complete</span>
                    <span class="metric-label">Status</span>
                </div>
                <div class="metric-sparkline status-sparkline"></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if data is not None and not data.empty:
            ...
            tab1, tab2, tab3, tab4 = st.tabs(["📋 Data Table", "📊 Visualizations", "📈 Statistics", "🔍 Insights"])
            
            with tab1:
                # Beautiful data table section
                st.markdown("""
                <div class="data-table-header">
                    <span class="table-icon">🗃️</span>
                    <span class="table-title">Data Explorer</span>
                    <span class="table-info">{} columns × {} rows</span>
                </div>
                """.format(len(data.columns), len(data)), unsafe_allow_html=True)
                ...
                for col in data.columns:
                    ...
                    if 'int' in dtype or 'float' in dtype:
                        col_types_html += f'<span class="col-badge col-numeric">🔢 {col}</span>'
                    elif 'datetime' in dtype or 'date' in dtype:
                        col_types_html += f'<span class="col-badge col-date">📅 {col}</span>'
                    else:
                        col_types_html += f'<span class="col-badge col-text">📝 {col}</span>'
                
                st.markdown(f"""
                <div class="column-badges-container">
                    {col_types_html}
                </div>
                """, unsafe_allow_html=True)
                
                # Enhanced dataframe with custom styling
                st.dataframe(
                    data,
            ...
            with tab2:
                # Beautiful visualizations section
                st.markdown("""
                <div class="viz-header">
                    <span class="viz-icon">📊</span>
                    <span class="viz-title">Data Visualizations</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                if len(data) > 1:
                    try:
                        ...
                        if len(numeric_cols) >= 2:
                            # Multi-chart layout for numeric data
                            ...
                            with viz_col1:
                                ...
                                fig_scatter.update_traces(marker=dict(size=10, line=dict(width=1, color='#8b5cf6')))
                                st.plotly_chart(fig_scatter, use_container_width=True)
                            
                            with viz_col2:
                                # Correlation heatmap if more than 2 numeric columns
                                if len(numeric_cols) >= 3:
                                    corr_matrix = data[numeric_cols].corr()
                                    fig_heat = px.imshow(
                                        corr_matrix,
                                        title="🔥 Correlation Heatmap",
                                        color_continuous_scale=['#0a0a1a', '#00d4ff', '#8b5cf6', '#ff2d92'],
                                        aspect='auto'
                                    )
                                    ...
                                    fig_line.update_traces(line=dict(color='#ff2d92', width=3))
                                    st.plotly_chart(fig_line, use_container_width=True)
                            
                            # Distribution plots
                            st.markdown("""
                            <div class="section-divider">
                                <span class="divider-text">📊 Distribution Analysis</span>
                            </div>
                            """, unsafe_allow_html=True)
                            
                            dist_cols = st.columns(min(3, len(numeric_cols)))
                            colors = ['#00d4ff', '#8b5cf6', '#ff2d92', '#39ff14', '#feca57']
                            ...
                            for i, col in enumerate(numeric_cols[:3]):
                                with dist_cols[i]:
                                    ...
                                    fig_hist.update_layout(
                                        ...
                                        paper_bgcolor='rgba(0,0,0,0)',
                                        font_color='#c0c0d5',
                                        title_font_color=colors[i % len(colors)],
                                        title_font_size=14,
                                        showlegend=False,
                                        xaxis=dict(gridcolor='rgba(0,212,255,0.1)'),
                                        yaxis=dict(gridcolor='rgba(0,212,255,0.1)')
                            ...
                            with viz_col1:
                                # Beautiful bar or histogram
                                if len(data) <= 20 and len(non_numeric_cols) > 0:
                                    fig_bar = px.bar(
                                        data,
                                        x=non_numeric_cols[0] if non_numeric_cols else data.index,
                                        y=numeric_cols[0],
                            ...
                            with viz_col2:
                                # Box plot for distribution insight
                                fig_box = px.box(
                                    data,
                                    y=numeric_cols[0],
                                    title=f"📦 {numeric_cols[0]} Box Plot",
                        ...
                        if len(non_numeric_cols) > 0 and len(data) <= 15:
                            ...
                            if len(value_counts) > 1:
                                st.markdown("""
                                <div class="section-divider">
                                    <span class="divider-text">🥧 Category Breakdown</span>
                                </div>
                                """, unsafe_allow_html=True)
                                ...
                                fig_pie = px.pie(
                                    ...
                                    color_discrete_sequence=['#00d4ff', '#8b5cf6', '#ff2d92', '#39ff14', '#feca57', '#ff6b6b', '#4ecdc4', '#a855f7']
                                )
                                fig_pie.update_layout(
                                    plot_bgcolor='rgba(10,10,25,0.8)',
                                    paper_bgcolor='rgba(0,0,0,0)',
                                    font_color='#c0c0d5',
                                    title_font_color='#ff2d92',
                                    title_font_size=16
                                )
                        ...
                        st.info("📊 Could not generate visualizations for this data type.")
                else:
                    st.info("📊 Need more than one row to generate visualizations.")
            ...
            with tab3:
                ...
                <div class="stats-header">
                    ...
                    <span class="stats-title">Statistical Analysis</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                if len(numeric_cols) > 0:
                    ...
                    # Display stats for each numeric column with beautiful cards
                    for col in numeric_cols[:6]:  # Limit to 6 columns for layout
                        col_stats = data[col].describe()
                        mean_val = col_stats['mean'] if 'mean' in col_stats else 0
                        std_val = col_stats['std'] if 'std' in col_stats else 0
                        min_val = col_stats['min'] if 'min' in col_stats else 0
                        max_val = col_stats['max'] if 'max' in col_stats else 0
                        median_val = col_stats['50%'] if '50%' in col_stats else 0
                        ...
                        <div class="stat-card-container">
                            ...
                            <div class="stat-card-grid">
                                <div class="stat-item">
                                    ...
                                    <span class="stat-value stat-mean">{mean_val:,.2f}</span>
                                </div>
                                <div class="stat-item">
                                    <span class="stat-label">Median</span>
                                    <span class="stat-value stat-median">{median_val:,.2f}</span>
                                </div>
                                <div class="stat-item">
                                    <span class="stat-label">Std Dev</span>
                                    <span class="stat-value stat-std">{std_val:,.2f}</span>
                                </div>
                                <div class="stat-item">
                                    <span class="stat-label">Min</span>
                                    <span class="stat-value stat-min">{min_val:,.2f}</span>
                                </div>
                                <div class="stat-item">
                                    <span class="stat-label">Max</span>
                                    <span class="stat-value stat-max">{max_val:,.2f}</span>
                                </div>
                                <div class="stat-item">
                                    <span class="stat-label">Range</span>
                                    <span class="stat-value stat-range">{(max_val - min_val):,.2f}</span>
                                </div>
                            </div>
                        </div>
                        """, unsafe_allow_html=True)
                    ...
                    st.markdown("""
                    <div class="section-divider">
                        <span class="divider-text">📋 Complete Statistics Table</span>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with tab4:
                ...
                <div class="insights-header">
                    ...
                    <span class="insights-title">Data Insights</span>
                </div>
                """, unsafe_allow_html=True)
                ...
                # Row count insight
                insights.append(f"📊 **Total Records:** {len(data):,} rows retrieved")
                insights.append(f"📋 **Columns:** {len(data.columns)} fields ({len(numeric_cols)} numeric, {len(non_numeric_cols)} categorical)")
                
                # Null value analysis
                null_counts = data.isnull().sum()
                if null_counts.any():
                    null_cols = null_counts[null_counts > 0]
                    insights.append(f"⚠️ **Missing Values:** Found in {len(null_cols)} column(s)")
                else:
                ...
                if len(numeric_cols) > 0:
                    for col in numeric_cols[:3]:
                        ...
                        if len(col_data) > 0:
                            ...
                            insights.append(f"📈 **{col}:** Ranges from {min_val:,.2f} to {max_val:,.2f} (avg: {mean_val:,.2f})")
                
                # Categorical insights
                if len(non_numeric_cols) > 0:
                    for col in non_numeric_cols[:2]:
            ...
            <div class="download-section">
                ...
                <span class="download-title">Export Data</span>
            </div>
            """, unsafe_allow_html=True)
            ...
            with dl_col3:
                # Excel-like format hint
                st.download_button(
                    label="📥 Download TSV",
                    data=data.to_csv(index=False, sep='\t'),
                    file_name=f"query_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.tsv",
